...
This sensor measures the volume of blood in the finger at a given moment and when the signal peaks and records a higher volume of blood we can estimate a heartbeat. After smoothing the signal using a moving mean, peaks are detected and validated by ensuring their amplitudes are large enough and the distance between peaks is possible. After finding these peaks, the distance between peaks is found to convert to BPM. Averaging a window of BPM history smooths out the BPM signal to classify into rested or stressed lights. 
